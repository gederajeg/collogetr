...
#'
collex_fye <- function(df, collstr_digit = 3, mpfr_precision = NULL) {
  collstr <- dplyr::quo(collstr)
  dP_collex_cue_cxn <- dplyr::quo(dP_collex_cue_cxn)
  dP_cxn_cue_collex <- dplyr::quo(dP_cxn_cue_collex)
  df <- dplyr::mutate(df, !!dplyr::quo_name(collstr) := purrr::map_dbl(data, fye_compute, mpfr_precision = mpfr_precision, collstr_digit = collstr_digit),
