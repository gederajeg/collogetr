...
The collocates are restricted to those occurring _one_ word (i.e. `span = 1L`) to the _right_ (`window = "r"`) of the verb. The `"r"` character in `window` stands for *right*-side collocates (`"l"` for *left*-side collocates and `"b"` for *both* right- and left-side collocates). The `span` argument requires integer to indicate the range of words covered in the specified window. The `pattern` argument requires one or more exact word forms. If more than one, put into a character vector (e.g., `c("mengatakan", "menjanjikan")`). The `save_interim` is `FALSE` means that no output is saved into the computer, but in the console (i.e., in the `out` object). 
